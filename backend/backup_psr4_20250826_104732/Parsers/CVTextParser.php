...
namespace Parsers\CVTextParser.php\Parsers;
...
class CVTextParser
{
    ...
    {
        preg_match("/(?:Direcci[oÃ³]n|Domicilio)[:\s]+(.+)/i", $this->text, $matches);
        return $matches[1] ?? '';
    }
    ...
    {
        preg_match("/(?:Formaci[oÃ³]n|Educaci[oÃ³]n|Estudios)[\s:]*([\s\S]{0,600})/iu", $this->text, $matches);
        return $matches[1] ?? '';
    }
