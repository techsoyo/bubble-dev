...
namespace Models\Skill.php\Models;

use Utils\Logger;

/**
 * Modelo para gestiÃ³n de habilidades (skills)
 * 
 * Gestiona las habilidades del sistema con soporte para categorizaciÃ³n,
 * niveles de competencia y extracciÃ³n desde texto.
 * 
 * @package Models
...
class Skill extends BaseModel
{
  ...
  /**
   * CategorÃ­as vÃ¡lidas de habilidades
   */
  ...
  /**
   * Contar habilidades segÃºn criterios
   * 
   * @param array $criteria Criterios de bÃºsqueda
   * @return int NÃºmero de habilidades encontradas
   */
   ...
   * 
   * @param string $text Texto del CV o descripciÃ³n
   * @return array Habilidades extraÃ­das
   */
  public function extractSkillsFromText(string $text): array
  {
    try {
      // Lista bÃ¡sica de habilidades tÃ©cnicas comunes
      $commonSkills = [
      ...
      foreach ($commonSkills as $skill) {
        if (strpos($textUpper, strtoupper($skill)) !== false) {
          $foundSkills[] = [
            ...
            'confidence' => 0.8 // Confidence score bÃ¡sico
          ];
        }
      }
  ...
  private function validateSkillData(array $data, bool $isCreation = true): void
  {
    ...
    if (isset($data['category']) && !in_array($data['category'], self::VALID_CATEGORIES)) {
      throw new \InvalidArgumentException('CategorÃ­a de habilidad no vÃ¡lida');
    }

    if (isset($data['level']) && !in_array($data['level'], self::VALID_LEVELS)) {
      throw new \InvalidArgumentException('Nivel de habilidad no vÃ¡lido');
    }
  }

  /**
   * Categorizar automÃ¡ticamente una habilidad
   * 
   * @param string $skillName Nombre de la habilidad
   * @return string CategorÃ­a asignada
   */
