...
/**
 * Endpoint para manejar callbacks de autenticaciÃ³n social (Google y LinkedIn)
 *
 * Este archivo procesa los cÃ³digos de autorizaciÃ³n OAuth2 y crea o actualiza
 * cuentas de usuario basadas en la informaciÃ³n del proveedor social.
 */
...
try {
    ...
    if ($error) {
        ...
        echo json_encode([
          ...
          'message' => 'El usuario cancelÃ³ la autenticaciÃ³n o hubo un error: ' . $error
        ]);
        exit;
    }

    // Verificar que se recibiÃ³ el cÃ³digo de autorizaciÃ³n
    if (!$code) {
        http_response_code(400);
        echo json_encode([
          'error' => 'CÃ³digo de autorizaciÃ³n faltante',
          'message' => 'No se recibiÃ³ el cÃ³digo de autorizaciÃ³n del proveedor'
        ]);
        exit;
    }
    ...
    switch ($provider) {
        ...
        case 'linkedin':
            ...
            break;
        default:
            throw new Exception('Proveedor de autenticaciÃ³n no soportado');
    }
    ...
    // Verificar si hay un job ID en el state para redirecciÃ³n
    $jobId = null;
    ...
    echo json_encode([
      ...
      'message' => 'No se pudo completar la autenticaciÃ³n. IntÃ©ntalo nuevamente.'
    ]);
}
...
function exchangeCodeForToken($provider, $code, $clientId, $clientSecret, $redirectUri)
{
    ...
    if (!$tokenData || !isset($tokenData['access_token'])) {
        throw new Exception('Token de acceso no vÃ¡lido');
    }
...
/**
 * Obtiene informaciÃ³n del usuario desde Google
 */
function getUserInfoFromGoogle($accessToken)
{
    ...
    if ($httpCode !== 200) {
        throw new Exception("Error al obtener informaciÃ³n del usuario de Google: HTTP $httpCode");
    }
...
/**
 * Obtiene informaciÃ³n del usuario desde LinkedIn
 */
function getUserInfoFromLinkedIn($accessToken)
{
    // Obtener perfil bÃ¡sico
    $ch = curl_init();
...
/**
 * Crea o actualiza un usuario desde autenticaciÃ³n social
 */
