<?php
// social-login.php - Manejo de autenticación OAuth con proveedores externos
session_start();
...
// Función para intercambiar código OAuth por tokens
function exchangeOAuthCode($provider, $code, $redirectUri)
{
  ...
  if (!$clientId || !$clientSecret) {
    throw new Exception("Configuración OAuth incompleta para $provider");
  }

  // Datos para el intercambio de código
  $postData = [
...
// Función para obtener datos del usuario desde el proveedor OAuth
function getUserFromProvider($provider, $accessToken)
{
  ...
  if (!$userData) {
    throw new Exception("Respuesta inválida del proveedor $provider");
  }
  ...
  switch ($provider) {
    ...
    case 'linkedin':
      $normalizedData = [
        ...
        'email' => '', // LinkedIn requiere petición separada para email
        'avatar' => null,
...
try {
  ...
  if (!$provider || !$code || !$redirectUri) {
    http_response_code(400);
    echo json_encode(['success' => false, 'message' => 'Parámetros OAuth incompletos']);
    exit();
  }

  if (!in_array($provider, ['google', 'linkedin', 'apple'])) {
    http_response_code(400);
    echo json_encode(['success' => false, 'message' => 'Proveedor OAuth no válido']);
    exit();
  }
  ...
  if ($existingUser) {
    ...
    // Insertar en tabla de usuarios para autenticación
    $userId = 'user_' . uniqid();
  ...
  // Paso 4: Establecer sesión
  $_SESSION['user_id'] = $user['id'];
  $_SESSION['user_email'] = $user['email'];
  $_SESSION['user_role'] = $user['role'];
  $_SESSION['login_time'] = time();

  // Generar token JWT si está configurado
  $jwtToken = null; // Se puede implementar más adelante
