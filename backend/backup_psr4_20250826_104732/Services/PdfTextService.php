...
namespace Services\PdfTextService.php\Services;

use RuntimeException;

/**
 * Servicio de extracciÃ³n de texto desde PDFs con doble estrategia:
 *  A) Texto "nativo" (pdftotext) + heurÃ­stica de calidad
 *  B) OCR (pdftoppm + tesseract) si habilitado y falla A
 *
 * DiseÃ±ado para el flujo de /api/cv/parse (currÃ­culums) garantizando:
 *  - Validaciones tempranas (existencia, tamaÃ±o, MIME, antivirus opcional)
 *  - Limpieza rigurosa de temporales
 *  - No exposiciÃ³n de rutas internas en excepciones
 *  - Logs mÃ­nimos estructurados sin contenido sensible
 */
class PdfTextService
{
    ...
    public function __construct()
    {
        ...
        // COMENTADO: Eliminamos dependencia de pdftotext - Llama procesarÃ¡ PDF directamente
    ...
    /**
     * NUEVO: MÃ©todo simplificado que retorna el path del PDF para procesamiento directo por Llama
    ...
    /**
     * DEPRECATED: Extrae texto usando heurÃ­sticas y OCR fallback.
     * @throws PdfSecurityException|PdfTextEmptyException|InfectedFileException
     */
    public function extract(string $pdfPath): string
    {
        // COMENTADO: MÃ©todo legacy - ahora Llama procesa PDF directamente
        ...
        // Ninguna estrategia Ãºtil
        throw new PdfTextEmptyException('E_PDF_TEXT_EMPTY');
        */
    }

    /* =================== Estrategias COMENTADAS =================== */
    // COMENTADO: MÃ©todos legacy que dependÃ­an de pdftotext
    /*
    ...
    */
    ...
    private function extractViaOcr(string $pdfPath, float $t0): ?string
    {
        ...
        // Orden natural por sufijo numÃ©rico
        ...
        foreach ($pngs as $img) {
            ...
            $this->execCommand($cmdOcr, 90); // OCR por pÃ¡gina
            $txtFile = $outBase . '.txt';
            if (is_file($txtFile)) {
    ...
    private function scoreTextQuality(string $txt): array
    {
        ...
        // Caracteres Ãºtiles: letras (incluye acentos) y dÃ­gitos
        $useful = preg_replace('/[^0-9A-Za-zÃÃ‰ÃÃ“ÃšÃœÃ‘Ã¡Ã©Ã­Ã³ÃºÃ¼Ã±]/u', '', $txt);
    ...
    private function normalizeUtfBlocks(string $txt): string
    {
        ...
        // 5. Limitar bloques de lÃ­neas vacÃ­as consecutivas a 2
        $out = [];
        $emptySeq = 0;
        foreach ($lines as $l) {
            if ($l === '') {
    ...
    private function scanIfEnabled(string $path): void
    {
        ...
        if ($res['exit'] !== 0) {
            // ClamAV tÃ­pico: exit 1 => infectado / otros cÃ³digos pueden variar, tratamos cualquiera !=0 como sospechoso
    ...
    {
        // ImplementaciÃ³n simple (sin proc_open complejo) dado entorno controlado.
