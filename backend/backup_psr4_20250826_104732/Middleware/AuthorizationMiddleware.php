...
namespace Middleware\AuthorizationMiddleware.php\Middleware;

use Utils\JWT;
use Security\SecurityLogger;

/**
 * Middleware de autorizaciÃ³n para endpoints administrativos
 * 
...
class AuthorizationMiddleware
{
   ...
   * @throws Exception Si la autenticaciÃ³n/autorizaciÃ³n falla
   */
  public static function requireAuth(array $requiredRoles = [], bool $strict = false): array
  {
    ...
    // Log autenticaciÃ³n exitosa
    SecurityLogger::logAuthSuccess($user['user_id'], $user['role'] ?? 'unknown');

    // Verificar autorizaciÃ³n si se requieren roles especÃ­ficos
    if (!empty($requiredRoles)) {
      ...
      if ($strict) {
        ...
        // Modo jerÃ¡rquico: admin puede acceder a todo
  ...
  /**
   * Verificar acceso a objeto especÃ­fico (Object Level Authorization)
   */
  public static function requireObjectAccess(string $resourceType, $resourceId, array $user = null): array
  {
    ...
    // Verificar si el usuario puede acceder a este recurso especÃ­fico
    switch ($resourceType) {
      case 'candidate':
      ...
      case 'application':
        // AquÃ­ implementarÃ­as lÃ³gica para verificar si el usuario
        // puede acceder a esta aplicaciÃ³n especÃ­fica
  ...
  /**
   * Rate limiting bÃ¡sico por usuario
   */
  ...
  private static function logSecurityEvent(string $event, array $details): void
  {
    ...
    // Para eventos crÃ­ticos, puedes implementar alertas adicionales
