...
namespace Utils\InputValidator.php\Utils;
...
class InputValidator
{
    ...
    public function validateAlphanumeric(mixed $value, string $fieldName = 'field'): bool
    {
        ...
        if (!preg_match('/^[a-zA-Z0-9_-]+$/', $v)) {
            $this->add($fieldName, "El campo $fieldName solo puede contener letras, nÃºmeros, guiones y guiones bajos");
            return false;
        }
        return true;
    }
    ...
    public function validateEmail(mixed $value, string $fieldName = 'email'): bool
    {
        ...
        if (filter_var($v, FILTER_VALIDATE_EMAIL) === false) {
            $this->add($fieldName, "El campo $fieldName debe ser un email vÃ¡lido");
            return false;
        }
        return true;
    }
    ...
    public function validateMaxLength(mixed $value, int $max, string $fieldName = 'field'): bool
    {
        $v = (string)($value ?? '');
        if ($this->mbLen($v) > $max) {
            $this->add($fieldName, "El campo $fieldName no puede tener mÃ¡s de $max caracteres");
            return false;
        }
        return true;
    }
    ...
    public function validateInteger(mixed $value, string $fieldName = 'field'): bool
    {
        if (filter_var($value, FILTER_VALIDATE_INT) === false) {
            $this->add($fieldName, "El campo $fieldName debe ser un nÃºmero entero");
            return false;
        }
        return true;
    }
    ...
    public function validateFloat(mixed $value, string $fieldName = 'field'): bool
    {
        if (filter_var($value, FILTER_VALIDATE_FLOAT) === false) {
            $this->add($fieldName, "El campo $fieldName debe ser un nÃºmero decimal");
            return false;
        }
        return true;
    }
    ...
    {
        if (!is_string($value)) {
            $this->add($fieldName, "El campo $fieldName debe ser un JSON vÃ¡lido");
            return false;
        }
    ...
    public function validateUrl(mixed $value, string $fieldName = 'url'): bool
    {
        ...
        if (!filter_var($v, FILTER_VALIDATE_URL)) {
            $this->add($fieldName, "El campo $fieldName debe ser una URL vÃ¡lida");
            return false;
        }
    ...
    {
        if (!is_numeric($value)) {
            $this->add($fieldName, "El campo $fieldName debe ser numÃ©rico");
            return false;
        }
        $num = $value + 0;
        if ($num < $min || $num > $max) {
    ...
    public function sanitizeText(mixed $value, int $maxLen = 2000): string
    {
        ...
        // 3) Normalizar saltos/tabs â†’ espacio
        $str = str_replace(["\r\n", "\r", "\n", "\t"], ' ', $str);
        ...
        // 9) NormalizaciÃ³n Unicode (si intl estÃ¡ cargado)
    ...
    /** Alias por compatibilidad histÃ³rica */
    ...
    /**
     * ValidaciÃ³n por reglas simples
