...
/**
 * Endpoint Ãºnico para anÃ¡lisis completo de CV
 * 
 * Soporta anÃ¡lisis de:
 * - Texto de CV (usando GroqApiService)  
 * - Archivos PDF (usando GroqApiService con extracciÃ³n de texto)
 ...
 * 
 * VersiÃ³n consolidada usando solo Groq (sin Ollama)
 * 
...
// âœ… REQUERIR AUTENTICACIÃ“N JWT SIEMPRE
$userPayload = JWTMiddleware::requireAuth();
if (!$userPayload) {
  // JWTMiddleware ya enviÃ³ la respuesta de error
  exit;
}

// âœ… CONFIGURACIÃ“N DE SEGURIDAD MEJORADA
define('MAX_FILE_SIZE', 5 * 1024 * 1024); // 5MB mÃ¡ximo (reducido)
define('MAX_TEXT_LENGTH', 512 * 1024); // 512KB mÃ¡ximo para texto
...
define('MAX_DAILY_UPLOADS', 10); // LÃ­mite diario por usuario
...
if (!checkUploadRateLimit($userId)) {
  ...
  echo json_encode([
    'success' => false,
    'message' => 'Has superado el lÃ­mite diario de subidas de CV (10 por dÃ­a)',
...
/**
 * Verificar lÃ­mite de subidas por usuario
 */
...
/**
 * Valida archivo subido segÃºn su tipo
 */
function validateUploadedFile($fileData, string $userId): array
{
  ...
  if ($fileData['error'] !== UPLOAD_ERR_OK) {
    $errors = [
      ...
      UPLOAD_ERR_FORM_SIZE => 'Archivo excede el lÃ­mite permitido',
      UPLOAD_ERR_PARTIAL => 'Archivo se subiÃ³ parcialmente',
      ...
      UPLOAD_ERR_EXTENSION => 'Archivo bloqueado por extensiÃ³n'
    ];
    return [
      'success' => false,
  ...
  // âœ… VALIDACIÃ“N DE TAMAÃ‘O ESTRICTA
  if ($fileData['size'] > MAX_FILE_SIZE) {
    return [
      'success' => false,
      'message' => 'Archivo demasiado grande. MÃ¡ximo ' . (MAX_FILE_SIZE / 1024 / 1024) . 'MB'
    ];
  }

  // âœ… VALIDACIÃ“N DE EXTENSIÃ“N Y MIME TYPE
  ...
  // âœ… VALIDACIÃ“N DE MIME TYPE REAL (no confiar en $_FILES)
  ...
  // âœ… VALIDACIÃ“N DE CONTENIDO PDF
  ...
  // âœ… SCAN DE VIRUS BÃSICO (buscar patrones sospechosos)
...
/**
 * Detectar contenido sospechoso bÃ¡sico
 */
...
function extractTextFromFile($filePath, $extension, $mime)
{
  switch ($extension) {
    ...
    case 'pdf':
      // Para PDFs, retornar null para usar anÃ¡lisis directo con Vision
      return null;

    case 'doc':
    case 'docx':
      // TODO: Implementar extracciÃ³n de Word si es necesario
      throw new Exception('AnÃ¡lisis de archivos Word no implementado aÃºn');
...
try {
  ...
  // Si hay error en JSON pero tambiÃ©n archivos subidos, continuar
  if (json_last_error() !== JSON_ERROR_NONE && empty($_FILES)) {
    ResponseHelper::error('JSON invÃ¡lido: ' . json_last_error_msg(), null);
  ...
  if (isset($_FILES['cv_file']) && $_FILES['cv_file']['error'] !== UPLOAD_ERR_NO_FILE) {
    ...
    if ($extension === 'pdf') {
      // AnÃ¡lisis directo de PDF con Vision
      $isPdfAnalysis = true;
  ...
  // === OPCIÃ“N 2: Texto directo en JSON ===
  elseif (isset($data['cv_text'])) {
    $cvText = $data['cv_text'];
    $processingMethod = 'groq-text-direct';

    if (strlen($cvText) > MAX_TEXT_LENGTH) {
      ResponseHelper::error('El texto del CV excede el tamaÃ±o mÃ¡ximo permitido.', null);
      http_response_code(400);
      exit;
    }
  }
  // === OPCIÃ“N 3: Archivo de texto por ruta ===
  elseif (isset($data['text_file_path'])) {
    ...
    ResponseHelper::error('No se proporcionÃ³ CV. Use cv_file, cv_text o text_file_path.', null);
    http_response_code(400);
    exit;
  }

  // === PROCESAMIENTO SEGÃšN TIPO ===

  $groqService = new GroqApiService();

  if ($isPdfAnalysis) {
    // AnÃ¡lisis de PDF con Groq (extrae texto del PDF internamente)
    try {
      $cvData = $groqService->analyzeCvFromPdf($filePath);
      $processingService = 'Groq API (Llama3) - PDF';
    } catch (Exception $e) {
      ResponseHelper::error('Error en anÃ¡lisis PDF: ' . $e->getMessage(), $e);
      http_response_code(500);
      exit;
    }
  } else {
    // AnÃ¡lisis de texto con Groq
    if (empty($cvText)) {
