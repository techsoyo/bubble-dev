...
try {
    ...
    if (!$email) {
        http_response_code(400);
        Res::error('Email invÃ¡lido', 400);
        exit;
    }
    
    if (strlen($password) < 6) {
        http_response_code(400);
        Res::error('La contraseÃ±a debe tener al menos 6 caracteres', 400);
        exit;
    }
    ...
    if ($action === 'login') {
        ...
        if ($existingUser) {
            ...
            Res::error('El usuario ya existe. Usa la opciÃ³n de login.', 409);
            exit;
        }
    ...
    http_response_code(500);
    Res::error('Error en autenticaciÃ³n de candidato', 500);
}
?>
