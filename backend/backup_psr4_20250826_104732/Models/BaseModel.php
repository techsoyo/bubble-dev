...
namespace Models\BaseModel.php\Models;
...
abstract class BaseModel
{
    ...
    public function __construct(?string $table = null, string|int $primaryKey = 'id')
    {
        ...
        if ($table !== null) {
            $this->table = T($table); // Usar la funciÃ³n T() para prefijos
        } elseif (empty($this->table)) {
            // Si no se proporciona tabla y no estÃ¡ definida, inferir del nombre de la clase
            $className = basename(str_replace('\\', '/', static::class));

            // Manejar clases anÃ³nimas
    ...
    public function store(array $data)
    {
        ...
        try {
            ...
            $this->invalidateCache(); // Invalidar cachÃ© despuÃ©s de crear
    ...
    public function update($id, array $data): bool
    {
        ...
        try {
            ...
            $this->invalidateCache(); // Invalidar cachÃ© despuÃ©s de actualizar
    ...
    public function query(string $sql, array $params = []): array
    {
        ...
        try {
            ...
            // Invalidar cachÃ© si es una operaciÃ³n de escritura
    ...
    {
        // Permite solo caracteres alfanumÃ©ricos y guiones bajos.
        if (preg_match('/^[a-zA-Z0-9_]+$/', $fieldName) !== 1) {
            return false;
        }

        // En un entorno de desarrollo, comprueba si el campo estÃ¡ en la lista de fillable o es la clave primaria.
        // Esto ayuda a detectar errores tipogrÃ¡ficos o intentos de usar campos no permitidos.
        if ($this->isDevelopment()) {
            if (!in_array($fieldName, $this->fillable) && $fieldName !== $this->primaryKey) {
                // PodrÃ­amos registrar una advertencia aquÃ­ si quisiÃ©ramos ser mÃ¡s estrictos.
