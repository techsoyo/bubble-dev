...
namespace Models\Notification.php\Models;
 ...
 * estados, cache optimizado y funcionalidades especÃ­ficas de notificaciones.
 * 
 * @package Models
...
class Notification extends BaseModel
{
     ...
     * 
     * Cambios realizados:
     * âž• Campos aÃ±adidos: ['title', 'is_read', 'data']
     * âŒ Campos removidos: ['status', 'sent_at', 'email_to', 'subject', 'error_message']
    ...
    /**
     * Tipos de notificaciÃ³n vÃ¡lidos segÃºn anÃ¡lisis de BD
     */
    const VALID_TYPES = [
        'general',
        'assignment_notice',
    ...
    /**
     * Estados vÃ¡lidos de notificaciÃ³n
     */
    ...
    /**
     * Obtener notificaciones no leÃ­das de un candidato con cache optimizado
     * 
     * @param string $candidateId ID del candidato
     * @param int $cacheTtl Tiempo de vida del cache en segundos
     * @return array Lista de notificaciones no leÃ­das
     */
    ...
    /**
     * Ejecutar query de notificaciones no leÃ­das
     * 
    ...
    /**
     * Marcar notificaciÃ³n como leÃ­da (enviada)
     * 
     * @param string $notificationId ID de la notificaciÃ³n
     * @return bool True si se marcÃ³ correctamente
     */
     ...
     * 
     * @param int $daysOld DÃ­as de antigÃ¼edad para considerar como "antiguas" (por defecto 30)
     * @param array $statusesToClean Estados a limpiar (por defecto solo 'sent')
     * @return int NÃºmero de notificaciones eliminadas
     */
    public function cleanupOldNotifications(int $daysOld = 30, array $statusesToClean = ['sent']): int
    {
        ...
        try {
            ...
            // Preparar condiciones para mÃºltiples estados
            ...
            if ($deletedCount > 0) {
                // Invalidar cache despuÃ©s de limpieza
    ...
    /**
     * Validar estado de notificaciÃ³n
     * 
     * @param string $status
     * @return bool
     */
    ...
    /**
     * Crear nueva notificaciÃ³n con validaciÃ³n
     * 
     * @param array $data Datos de la notificaciÃ³n
     * @return mixed ID de la nueva notificaciÃ³n o false
     */
    ...
    /**
     * MÃ‰TODOS HEREDADOS DEL MODELO ORIGINAL - Mantener compatibilidad
     */
    ...
    /**
     * Obtener estadÃ­sticas de notificaciones por candidato
     * 
     * @param string $candidateId
     * @return array EstadÃ­sticas (total, pending, sent, failed)
     */
    ...
    /**
     * Invalidar cache especÃ­fico de notificaciones
     * 
     * @param string|null $notificationId ID especÃ­fico de notificaciÃ³n (opcional)
     * @return int NÃºmero de entradas de cache eliminadas
     */
    ...
    /**
     * Generar clave de cache para notificaciones (usa mÃ©todo de BaseModel)
     * 
     * @param string $operation Tipo de operaciÃ³n
     * @param array $params ParÃ¡metros para la clave
    ...
    /**
     * Crear nueva notificaciÃ³n con validaciones CRUD estÃ¡ndar
     *
     * @param array $data Datos de la notificaciÃ³n
     * @return int|false ID de la nueva notificaciÃ³n o false en caso de error
    ...
    public static function createNotificationStandard(array $data)
    {
        ...
        if (!$id) {
            throw new \Exception('Error al crear la notificaciÃ³n');
        }
     ...
     *
     * @param int $id ID de la notificaciÃ³n
     * @return array|null
     */
    public static function getNotificationStandard(int $id): ?array
    {
        try {
            ...
            self::logError('Error al obtener notificaciÃ³n', ['id' => $id], $e);
            return null;
        }
    }

    /**
     * Actualizar notificaciÃ³n con validaciones
     *
     * @param int $id ID de la notificaciÃ³n
    ...
    public static function updateNotificationStandard(int $id, array $data): bool
    {
        ...
        if (!$existingNotification) {
            throw new \InvalidArgumentException("NotificaciÃ³n con ID {$id} no encontrada");
        }
    ...
    public static function deleteNotificationStandard(int $id): bool
    {
        try {
            ...
            self::logError('Error al eliminar notificaciÃ³n', ['id' => $id], $e);
            return false;
        }
    }
    ...
    public static function searchNotificationsStandard(array $criteria = [], int $limit = 50, int $offset = 0): array
    {
        try {
            ...
            self::logError('Error en bÃºsqueda de notificaciones', $criteria, $e);
            return [];
        }
    }
    ...
    {
        // candidate_id es requerido en creaciÃ³n
        ...
        if (isset($data['candidate_id']) && (!is_numeric($data['candidate_id']) || $data['candidate_id'] <= 0)) {
            throw new \InvalidArgumentException('El candidate_id debe ser un nÃºmero entero positivo');
        }

        // type es requerido en creaciÃ³n
        ...
        if (isset($data['type'])) {
            if (!in_array($data['type'], self::VALID_TYPES)) {
                throw new \InvalidArgumentException('Tipo de notificaciÃ³n no vÃ¡lido: ' . $data['type']);
            }
        }
        ...
        if (isset($data['title'])) {
            if (!is_string($data['title']) || strlen(trim($data['title'])) < 1) {
                throw new \InvalidArgumentException('El tÃ­tulo no puede estar vacÃ­o');
            }
        ...
        // message es requerido en creaciÃ³n
        ...
        // Validar data (debe ser un array vÃ¡lido o JSON vÃ¡lido)
        if (isset($data['data'])) {
            if (is_string($data['data'])) {
                json_decode($data['data']);
                if (json_last_error() !== JSON_ERROR_NONE) {
                    throw new \InvalidArgumentException('El campo data debe contener JSON vÃ¡lido');
                }
            } elseif (!is_array($data['data']) && !is_null($data['data'])) {
                throw new \InvalidArgumentException('El campo data debe ser un array o JSON vÃ¡lido');
            }
        }
    }

    /**
     * Invalidar cachÃ© relacionado con notificaciones (versiÃ³n estÃ¡tica)
     */
    private static function invalidateNotificationCacheStandard(): void
    {
        try {
            ...
            self::logDebug('CachÃ© de notificaciones invalidado');
        } catch (\Exception $e) {
            self::logError('Error al invalidar cachÃ© de notificaciones', [], $e);
        }
    }
}
