...
class SocialLogin extends BaseModel
{
     ...
     * 
     * Cambios realizados:
     * ➕ Campos añadidos: ['candidate_id', 'provider_user_id']
     * ❌ Campos removidos: ['user_id', 'provider_id', 'access_token', 'refresh_token', 'expires_at', 'profile_data']
    ...
    /**
     * Clave de encriptación para tokens (debería venir de configuración)
     */
    ...
    public function findByProvider(string $provider, string $providerId): ?array
    {
        ...
        if (empty($providerId)) {
            throw new \InvalidArgumentException('Provider ID no puede estar vacío');
        }
    ...
    public function refreshToken(int $socialLoginId, string $newAccessToken, ?string $newRefreshToken = null, ?string $expiresAt = null): bool
    {
        if (empty($newAccessToken)) {
            throw new \InvalidArgumentException('Access token no puede estar vacío');
        }
    ...
    /**
     * Obtener estadísticas de proveedores OAuth
     *
     * @return array Estadísticas de uso por proveedor
     */
