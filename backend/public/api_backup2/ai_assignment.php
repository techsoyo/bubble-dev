...
// helpers mÃ­nimos si tu bootstrap no los define
...
try {
    // exige token vÃ¡lido (recruiter/sistema)
    JWT::requireAuth();

    $payload = Request::json();

    // ValidaciÃ³n fuerte con tu Validator (sin Val::int inexistente)
    ['ok' => $ok, 'errors' => $errors] = Val::validate($payload, [
        'candidate_id' => 'required|string:1,36|regex:/^cnd-\d+$/',
        'skills'       => 'required|array',
    ]);
    if (!$ok) {
        Res::error('ValidaciÃ³n fallida', 422, ['errors' => $errors]);
    }
    ...
    if (!$skills) {
        Res::error('skills debe ser array no vacÃ­o', 422);
    }
    ...
    // Fallback simple (ajÃºstalo a tu dominio)
    if (!$categoryId || !$departmentId) {
        // ejemplo de heurÃ­stica mÃ­nima
        $skillMap = [
    ...
    // 2) SelecciÃ³n automÃ¡tica de recruiter (si tienes vista/tabla de carga, Ãºsala)
    $recruiterId = null;
    try {
  ...
  ");
    ...
    Res::success('AsignaciÃ³n realizada', [
