...
try {
  ...
  if (!$input) {
    ...
    echo json_encode(['success' => false, 'message' => 'Datos invÃ¡lidos: ' . json_last_error_msg()]);
    exit;
  }
  ...
  if ($newPassword !== $newPasswordConfirmation) {
    http_response_code(400);
    echo json_encode(['success' => false, 'message' => 'Las contraseÃ±as nuevas no coinciden']);
    exit;
  }
  ...
  if (!password_verify($currentPassword, $candidate['password_hash'])) {
    http_response_code(400);
    echo json_encode(['success' => false, 'message' => 'La contraseÃ±a actual es incorrecta']);
    exit;
  }
  ...
  if ($result) {
    ...
    echo json_encode(['success' => false, 'message' => 'Error al actualizar la contraseÃ±a']);
  }
} catch (Exception $e) {
