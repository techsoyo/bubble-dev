...
namespace Controllers\AuthController.php\Controllers;
 ...
 * - NO hacer require de backend/public/api/bootstrap.php aquÃ­.
 * - La autenticaciÃ³n por cookie HttpOnly se aplica en endpoints/routers con JWTMiddleware.
 * - AquÃ­ solo generamos/limpiamos cookies y exponemos utilidades (user-info, verify, etc.).
 */
class AuthController
{
  public function __construct()
  {
    // En producciÃ³n no aceptamos Authorization header: solo cookie HttpOnly
  ...
  {
    // AquÃ­ irÃ­a la implementaciÃ³n real para validar credenciales
    $user = null; // Resultado de consulta a base de datos
    
    // Si no se encuentra el usuario o la contraseÃ±a no coincide
    if ($user === null) {
      return null;
    }
    ...
    // Sin implementaciÃ³n real -> null (no usar mocks)
    // return null; // CÃ³digo inalcanzable
  }
  ...
  /**
   * Login genÃ©rico (si lo usas en tu router). Recomendado usar los especÃ­ficos.
   */
  ...
  /**
   * InformaciÃ³n del usuario autenticado (claims del JWT)
   */
  ...
  /**
   * Verifica si la sesiÃ³n (cookie JWT) es vÃ¡lida
   */
  ...
  /**
   * ValidaciÃ³n bÃ¡sica de CSRF (double-submit) â€” preferible usar CsrfMiddleware en endpoints de escritura
   */
  ...
  /**
   * Cambio de contraseÃ±a â€” requiere JWT (debe validarse en middleware)
   * ImplementaciÃ³n real pendiente (DAO).
   */
  ...
  /**
   * Interceptor de autenticaciÃ³n (debug)
   */
