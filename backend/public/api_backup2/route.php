 ...
 * Objetivo: Ruteo automÃ¡tico de candidatos a reclutadores segÃºn reglas configurables
 *
 * Input: {
 *   "candidate_id": "string",
...
try {
  ...
  if ($routingRules['prefer_department_by_skills'] ?? true) {
    ...
    if (!empty($candidateSkills)) {
      // Buscar mapeo skills â†’ departamento
  ...
  // Estrategia 2: Departamento por defecto basado en categorÃ­a
  if (!$departmentId && isset($candidateData['categoria'])) {
    $categoria = strtolower($candidateData['categoria']);

    // Mapeo bÃ¡sico categorÃ­a â†’ departamento
    $categoryMapping = [
