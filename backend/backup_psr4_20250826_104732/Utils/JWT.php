...
namespace Utils\JWT.php\Utils;

/**
 * Clase segura para manejo de tokens JWT
 *
 * Implementa mejores prÃ¡cticas de seguridad para la generaciÃ³n y validaciÃ³n
...
class JWT
{
     ...
     */
    // Reducir el tiempo de expiraciÃ³n por defecto a 3600 segundos (1 hora).
    // Tiempos de expiraciÃ³n mÃ¡s cortos limitan la ventana de explotaciÃ³n en caso de
    // compromiso de un token.  Los tokens de larga duraciÃ³n incrementan el riesgo si se
    ...
    /**
     * Tiempo de tolerancia para validaciÃ³n de tiempo (5 minutos)
     */
     ...
     * @param int|null $expiry Tiempo de expiraciÃ³n en segundos
     * @param string $algorithm Algoritmo de firma
     * @return string Token JWT
     * @throws Exception Si hay errores en la configuraciÃ³n
     */
    public static function generate($payload, $expiry = null, $algorithm = 'HS256')
    {
        ...
        // Si no se especifica un tiempo de expiraciÃ³n, usar el valor por defecto
        if ($expiry === null) {
            $expiry = config('JWT_EXPIRY', self::$defaultExpiry);
        }

        // Validar tiempo de expiraciÃ³n
        if ($expiry <= 0 || $expiry > 86400 * 30) { // MÃ¡ximo 30 dÃ­as
            throw new \Exception('Tiempo de expiraciÃ³n invÃ¡lido');
        }

        $currentTime = time();

        // Crear header con informaciÃ³n adicional de seguridad
        $header = [
          'alg' => $algorithm,
          'typ' => 'JWT',
          'kid' => self::getKeyId() // Key ID para rotaciÃ³n de claves
        ];

        // AÃ±adir claims estÃ¡ndar al payload
        ...
        $payload['jti'] = self::generateJti(); // JWT ID Ãºnico
        ...
        // Log para auditorÃ­a (sin informaciÃ³n sensible)
     ...
     * @param array $options Opciones de validaciÃ³n
     * @return array|false Payload decodificado o false si es invÃ¡lido
     */
    public static function verify($token, $options = [])
    {
        try {
            ...
            // Validar claims estÃ¡ndar
            if (!self::validateStandardClaims($payload, $options)) {
                return false;
            }

            // Validar que el token no estÃ© en blacklist (si se implementa)
            ...
            // Log de Ã©xito (sin informaciÃ³n sensible)
            if (class_exists('\Utils\Logger')) {
    ...
    /**
     * Invalida un token aÃ±adiÃ©ndolo a una blacklist
     *
     * @param string $token Token a invalidar
     * @return bool True si se invalidÃ³ correctamente
     */
     ...
     * @throws Exception Si la clave no estÃ¡ configurada
     */
    private static function getSecret()
    {
        ...
        if (empty($secret)) {
            throw new \Exception('JWT_SECRET no estÃ¡ configurado');
        }
    ...
    /**
     * Obtiene el ID de la clave para rotaciÃ³n
     *
    ...
    /**
     * Valida los claims estÃ¡ndar del JWT
     *
     * @param array $payload Payload decodificado
     * @param array $options Opciones de validaciÃ³n
     * @return bool True si es vÃ¡lido
     ...
     * @return bool True si estÃ¡ en blacklist
     */
    private static function isTokenBlacklisted($jti)
    {
        // ImplementaciÃ³n simple con archivos (en producciÃ³n usar Redis/DB)
    ...
    /**
     * AÃ±ade un token a la blacklist
     *
     * @param string $jti JWT ID
     * @param int $exp Tiempo de expiraciÃ³n
     * @return bool True si se aÃ±adiÃ³ correctamente
     */
