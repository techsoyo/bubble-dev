...
try {
  // Solo permitir mÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©todo POST
  ...
  if (!$input) {
    ...
    echo json_encode(['success' => false, 'message' => 'Datos invÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡lidos: ' . json_last_error_msg()]);
    exit;
  }
  ...
  // Validar que las nuevas contraseÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â±as coincidan
  if ($newPassword !== $newPasswordConfirmation) {
    http_response_code(400);
    echo json_encode(['success' => false, 'message' => 'Las contraseÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â±as nuevas no coinciden']);
    exit;
  }

  // Validar longitud mÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­nima de la nueva contraseÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â±a
  if (strlen($newPassword) < 6) {
    http_response_code(400);
    echo json_encode(['success' => false, 'message' => 'La nueva contraseÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â±a debe tener al menos 6 caracteres']);
    exit;
  }
  ...
  // Verificar contraseÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â±a actual
  if (!password_verify($currentPassword, $candidate['password_hash'])) {
    http_response_code(400);
    echo json_encode(['success' => false, 'message' => 'La contraseÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â±a actual es incorrecta']);
    exit;
  }

  // Generar hash para la nueva contraseÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â±a
  $newPasswordHash = password_hash($newPassword, PASSWORD_DEFAULT);

  // Actualizar la contraseÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â±a
  ...
  if ($result) {
    echo json_encode([
      'success' => true,
      'message' => 'ContraseÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â±a actualizada correctamente',
      'data' => [
    ...
    echo json_encode(['success' => false, 'message' => 'Error al actualizar la contraseÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â±a']);
  }
} catch (Exception $e) {
