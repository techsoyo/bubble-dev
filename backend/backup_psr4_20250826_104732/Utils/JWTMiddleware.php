...
namespace Utils\JWTMiddleware.php\Utils;

/**
 * JWT Middleware - ProtecciÃ³n de endpoints con JWT
 * 
 * @package Utils
...
{
  /**
   * Verificar autenticaciÃ³n JWT en request actual
   */
  public static function requireAuth(): ?array
  {
    $token = null;

    // En producciÃ³n: solo cookies por seguridad
    if (($_ENV['APP_ENV'] ?? 'development') === 'production') {
      $token = Cookies::getJwt();

      if (!$token) {
        self::sendUnauthorized('Token de autorizaciÃ³n requerido');
        return null;
      }
    } else {
      // En desarrollo/staging: mantener fallback para compatibilidad
      // 1. Primero, intentar obtener token desde cookie (mÃ©todo preferido)
      $token = Cookies::getJwt();
    ...
    if (!$payload) {
      self::sendUnauthorized('Token invÃ¡lido o expirado');
      return null;
    }
