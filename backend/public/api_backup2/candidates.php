...
// ConfiguraciÃ³n de seguridad especÃ­fica para candidatos
$securityConfig = [
    'rate_limit_type' => 'default',
    'allowed_methods' => ['GET', 'POST', 'PUT', 'DELETE', 'OPTIONS'],
    'max_size' => 2097152, // 2MB
    'csrf_protection' => false // HabilitarÃ­amos en producciÃ³n con frontend preparado
];

// Aplicar middleware de seguridad

// Si tienes un middleware de seguridad real, colÃ³calo aquÃ­. Si no, omite esta llamada.
...
/**
 * Obtiene todos los candidatos con paginaciÃ³n
 */
...
");
    ...
    // Bind parameters con tipos especÃ­ficos
...
{
    $input = ResponseHelper::getJsonInput(1048576); // 1MB mÃ¡ximo
    if ($input === null) {
        return; // El ResponseHelper ya enviÃ³ el error
    }

    // ValidaciÃ³n exhaustiva con middleware mejorado
    $validationRules = [
        'name' => [
            'required' => true,
            'min_length' => 2,
    ...
    // Validar con middleware de validaciÃ³n mejorado
    if (!ValidationMiddleware::handle($input, $validationRules)) {
        return; // El middleware ya enviÃ³ la respuesta de error
    }

    // Validar archivos si estÃ¡n presentes
    if (!empty($_FILES)) {
        $fileRules = [
            'cv_file' => [
                'required' => false,
        ...
        if (!empty($fileErrors)) {
            Logger::warning('ValidaciÃ³n de archivos fallida al crear candidato', $fileErrors);
            ResponseHelper::error('Error de validaciÃ³n de archivos', 422, $fileErrors);
            return;
        }
    }

    // Los datos ya estÃ¡n validados y seguros para usar
    $validData = $input;
    ...
    if ($emailCheck->fetch()) {
        ...
        ResponseHelper::error('El email ya estÃ¡ registrado', 409);
        return;
    }
...
{
    if (!isset($_GET['id'])) {
        ResponseHelper::error('ID requerido para actualizaciÃ³n', 400);
        return;
    }
    ...
    // ValidaciÃ³n para actualizaciÃ³n con middleware mejorado (campos opcionales)
    ...
    // ValidaciÃ³n adicional para skills si estÃ¡ presente
    if (isset($input['skills'])) {
        ...
        foreach ($input['skills'] as $index => $skill) {
            ...
            if (!$skillValidation['isValid']) {
                ResponseHelper::error("Skill en posiciÃ³n $index es invÃ¡lida: " . $skillValidation['error'], 422);
                return;
            }

            if (strlen($skill) > 100) {
                ResponseHelper::error("Skill en posiciÃ³n $index excede 100 caracteres", 422);
                return;
            }
        }
    }
...
{
    if (!isset($_GET['id'])) {
        ResponseHelper::error('ID requerido para eliminaciÃ³n', 400);
        return;
    }
...
function sanitizeCandidateOutput($candidate)
{
    return [
        ...
        'title' => htmlspecialchars($candidate['department_category_name'] ?? 'Sin categorÃ­a', ENT_QUOTES, 'UTF-8'),
