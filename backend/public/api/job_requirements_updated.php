...
try {
  ...
  if ($jobId) {
    $stmt = $db->prepare("SELECT id, job_id, requirement, created_at, updated_at 
                             FROM bt_job_requirements 
                             WHERE job_id = :jobId 
                             ORDER BY id ASC 
  ...
  if (empty($list) && ($_ENV['APP_ENV'] ?? 'production') === 'development') {
    $fallbackData = [
      ['id' => 1, 'job_id' => 1, 'requirement' => 'Experiencia mínima 3 años'],
      ['id' => 2, 'job_id' => 1, 'requirement' => 'Dominio de JavaScript'],
      ['id' => 3, 'job_id' => 1, 'requirement' => 'Conocimiento en frameworks modernos']
    ];
