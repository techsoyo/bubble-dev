...
namespace Models\User.php\Models;
 ...
 * siguiendo los nuevos patrones de la aplicaciÃ³n. Mantiene toda la funcionalidad
 * especÃ­fica existente mientras adopta las mejoras de rendimiento, cache y 
 * manejo de errores del BaseModel.
 *
 * Cambios principales en la migraciÃ³n:
 * - ConfiguraciÃ³n correcta de tabla con prefijo automÃ¡tico
 * - ActualizaciÃ³n de fillable y hidden segÃºn especificaciones BD
 * - ImplementaciÃ³n de cache en mÃ©todos costosos
 * - AdopciÃ³n de patrones de manejo de errores del BaseModel
 * - Mantenimiento completo de funcionalidad de autenticaciÃ³n existente
 *
 * Features:
...
{
    /**
     * MIGRADO: Tabla configurada para usar prefijo automÃ¡tico bt_users
     * La funciÃ³n T() aÃ±adirÃ¡ automÃ¡ticamente el prefijo 'bt_' para bt_users
     *
     * @var string
     */
     ...
     * âŒ Campos removidos: ['name', 'email', 'password', 'role', 'status', 'avatar', 'phone', 'bio', 'company', 'position']
     * ðŸ“Š Total campos fillable: 0
     * 
    ...
    /**
     * MIGRADO: Campos fillable actualizados segÃºn especificaciones de BD
     * Incluye todos los campos requeridos para el modelo User segÃºn anÃ¡lisis
     *
     * @var array<string>
     */
    ...
    /**
     * MIGRADO: Campos hidden actualizados segÃºn especificaciones de seguridad
     ...
     * heredadas del BaseModel. Utiliza el mÃ©todo findById del padre con
     * logging especÃ­fico de seguridad para usuarios.
     *
     ...
     *
     * MIGRADO: Mantiene funcionalidad de autenticaciÃ³n crÃ­tica
    ...
    /**
     * MIGRADO: VersiÃ³n optimizada con cache para bÃºsquedas frecuentes
     ...
     *
     * MIGRADO: Mantiene funcionalidad de validaciÃ³n de unicidad
     ...
     *
     * MIGRADO: Mantiene funcionalidad de creaciÃ³n segura con mejoras
     * Creates a new user account with automatic password hashing, email validation,
     * and comprehensive security logging. Utiliza el mÃ©todo store del BaseModel
     * con validaciones adicionales especÃ­ficas de usuarios.
     *
     ...
     *
     * MIGRADO: Mantiene funcionalidad de actualizaciÃ³n con mejoras de cache
     ...
     *
     * MIGRADO: Mantiene funcionalidad crÃ­tica de autenticaciÃ³n
     ...
     *
     * MIGRADO: Mantiene funcionalidad de recuperaciÃ³n de contraseÃ±a
     ...
     *
     * MIGRADO: Mantiene funcionalidad de actualizaciÃ³n de contraseÃ±a
     ...
     *
     * MIGRADO: VersiÃ³n mejorada con cache para mejor rendimiento
     ...
     *
     * MIGRADO: VersiÃ³n optimizada con cache del BaseModel
    ...
    /**
     * MIGRADO: Nuevo mÃ©todo - Get user statistics with caching
     *
    ...
    /**
     * MIGRADO: Nuevo mÃ©todo - Calculate user statistics from database
     *
    ...
    /**
     * MIGRADO: Nuevo mÃ©todo - Deactivate a user account
     *
    ...
    /**
     * MIGRADO: Nuevo mÃ©todo - Override getLargeColumns to exclude large user data fields
     *
     * Define campos grandes que deben excluirse de consultas estÃ¡ndar para optimizar rendimiento
     *
    ...
    /**
     * MIGRADO: Nuevo mÃ©todo - Invalidar cache especÃ­fico de usuarios
     *
     * Limpia el cache relacionado con usuarios cuando se realizan cambios
     * importantes que afectan las consultas cacheadas.
     * MÃ‰TODOS CRUD ENCAPSULADOS ADICIONALES - Para uso externo
     * Complementan los mÃ©todos ya existentes (store, update estÃ¡n implementados)
     */
    ...
    public static function searchUsers(array $criteria = [], int $limit = 50, int $offset = 0): array
    {
        try {
            ...
            self::logError('Error en bÃºsqueda de usuarios', $criteria, $e);
            return [];
        }
    }
    ...
    {
        // email es requerido en creaciÃ³n
        ...
        if (isset($data['email'])) {
            if (!filter_var($data['email'], FILTER_VALIDATE_EMAIL)) {
                throw new \InvalidArgumentException('El email no tiene un formato vÃ¡lido');
            }
        ...
        // name es requerido en creaciÃ³n
        ...
        // password es requerido en creaciÃ³n
        if (!$isUpdate && empty($data['password'])) {
            throw new \InvalidArgumentException('La contraseÃ±a es requerida');
        }
        ...
        if (isset($data['password'])) {
            ...
            if (strlen($data['password']) > 255) {
                throw new \InvalidArgumentException('La contraseÃ±a no puede exceder los 255 caracteres');
            }
        }
        ...
        if (isset($data['role'])) {
            ...
            if (!in_array($data['role'], $validRoles)) {
                throw new \InvalidArgumentException('Rol no vÃ¡lido: ' . $data['role']);
            }
        }
    ...
    /**
     * Invalidar cachÃ© relacionado con usuarios
     */
    private static function invalidateUserCache(): void
    {
        try {
            // Como los mÃ©todos CRUD son estÃ¡ticos, necesitamos crear una instancia temporal
            $tempInstance = new self();

            // Limpiar todo el cachÃ© de la instancia
            $tempInstance->cache = [];

            self::logDebug('CachÃ© de usuarios invalidado');
        } catch (\Exception $e) {
            self::logError('Error al invalidar cachÃ© de usuarios', [], $e);
        }
    }
}
