...
namespace Models\DepartmentCategory.php\Models;

use Utils\Logger;

/**
 * Modelo para las categorÃ­as dentro de cada departamento con soporte jerÃ¡rquico
 *
 * Proporciona funcionalidad completa para manejar categorÃ­as de departamentos
 * con estructura jerÃ¡rquica, cache optimizado y validaciones.
 *
 * @package Models
...
class DepartmentCategory extends BaseModel
{
     ...
     * 
     * Cambios realizados:
     * âž• Campos aÃ±adidos: ['department_id']
     * âŒ Campos removidos: ['description', 'parent_id', 'sort_order', 'status']
     * ðŸ“Š Total campos fillable: 2
     * 
    ...
    /**
     * Cache TTL para jerarquÃ­as de categorÃ­as en segundos
     */
    private const HIERARCHY_CACHE_TTL = 3600; // 1 hora

    /**
     * Cache TTL para categorÃ­as con departamentos en segundos
     */
    private const DEPARTMENT_CACHE_TTL = 1800; // 30 minutos

    /**
     * Estados vÃ¡lidos para categorÃ­as
     */
    private const VALID_STATUSES = ['active', 'inactive', 'draft'];

    /**
     * Devuelve las categorÃ­as de un departamento concreto (mÃ©todo original)
     *
    ...
    /**
     * Obtiene la estructura jerÃ¡rquica completa de categorÃ­as
     *
     * Construye un Ã¡rbol jerÃ¡rquico de todas las categorÃ­as activas
     * con soporte de cache para optimizar rendimiento.
     *
     * @param bool $includeInactive Incluir categorÃ­as inactivas
     * @param int $cacheTtl TTL del cache en segundos
     * @return array Estructura jerÃ¡rquica de categorÃ­as
     */
    ...
    /**
     * Obtiene una categorÃ­a con sus departamentos relacionados
     *
     * Incluye informaciÃ³n completa de la categorÃ­a junto con
     * todos los departamentos que pertenecen a ella.
     *
     * @param int $categoryId ID de la categorÃ­a
     * @param int $cacheTtl TTL del cache en segundos
     * @return array|null CategorÃ­a con departamentos o null si no existe
     */
    ...
    /**
     * Obtiene las categorÃ­as de nivel superior (sin parent)
     *
     * Devuelve solo las categorÃ­as raÃ­z que no tienen categorÃ­a padre,
    ...
    /**
     * Obtiene candidatos agrupados por categorÃ­a usando vistas de BD
     *
     * Utiliza las vistas optimizadas para obtener estadÃ­sticas de candidatos
     * por categorÃ­a de departamento.
     *
     * @param int|null $categoryId ID especÃ­fico de categorÃ­a (null para todas)
     * @param array $filters Filtros adicionales para candidatos
     * @return array Candidatos agrupados por categorÃ­a
     */
    ...
    /**
     * Valida la estructura jerÃ¡rquica antes de guardar
     *
     * Previene loops circulares y valida la integridad de la jerarquÃ­a.
     *
     * @param int|null $parentId ID de la categorÃ­a padre
     * @param int|null $currentId ID de la categorÃ­a actual (para updates)
     * @return bool True si la estructura es vÃ¡lida
     * @throws \InvalidArgumentException Si la estructura no es vÃ¡lida
     */
    public function validateHierarchy(?int $parentId, ?int $currentId = null): bool
    {
        if ($parentId === null) {
            return true; // CategorÃ­a raÃ­z, siempre vÃ¡lida
        }

        if ($parentId === $currentId) {
            throw new \InvalidArgumentException('Una categorÃ­a no puede ser padre de sÃ­ misma');
        }

        try {
            // Verificar que el padre existe y estÃ¡ activo
            $parent = $this->findById($parentId);
            if (!$parent) {
                throw new \InvalidArgumentException('La categorÃ­a padre especificada no existe');
            }

            if ($parent['status'] !== 'active') {
                throw new \InvalidArgumentException('La categorÃ­a padre debe estar activa');
            }
            ...
            if ($currentId !== null) {
                ...
                if (in_array($currentId, array_column($ancestors, 'id'))) {
                    throw new \InvalidArgumentException('La jerarquÃ­a propuesta crearÃ­a un bucle circular');
                }
            }

            // Validar profundidad mÃ¡xima (por ejemplo, 5 niveles)
            $depth = $this->getCategoryDepth($parentId);
            if ($depth >= 5) {
                throw new \InvalidArgumentException('La profundidad mÃ¡xima de jerarquÃ­a es 5 niveles');
            }
    ...
    /**
     * Crea o actualiza una categorÃ­a con validaciones
     *
     * @param array $data Datos de la categorÃ­a
     * @param int|null $id ID para actualizaciÃ³n (null para crear)
     * @return mixed ID de la categorÃ­a creada o true para actualizaciÃ³n
    ...
    public function createOrUpdate(array $data, ?int $id = null)
    {
        ...
        if (isset($data['status']) && !in_array($data['status'], self::VALID_STATUSES)) {
            throw new \InvalidArgumentException('Estado no vÃ¡lido. Debe ser: ' . implode(', ', self::VALID_STATUSES));
        }
    ...
    /**
     * Elimina una categorÃ­a y reorganiza la jerarquÃ­a
     *
     * @param int $id ID de la categorÃ­a a eliminar
    ...
    public function deleteWithHierarchy(int $id, bool $promoteChildren = true): bool
    {
        ...
        try {
            // Obtener la categorÃ­a a eliminar
            $category = $this->findById($id);
            if (!$category) {
                throw new \InvalidArgumentException('Category not found');
            }

            // Obtener categorÃ­as hijas
    ...
    /**
     * Construye la estructura jerÃ¡rquica de categorÃ­as
     */
    ...
    /**
     * Construye Ã¡rbol jerÃ¡rquico desde lista plana
     */
    ...
    /**
     * Ejecuta query para obtener categorÃ­a con departamentos
     */
    ...
    /**
     * Obtiene los ancestros de una categorÃ­a
     */
    ...
    /**
     * Calcula la profundidad de una categorÃ­a en la jerarquÃ­a
     */
    ...
    /**
     * Invalida el cache especÃ­fico de categorÃ­as
     */
    ...
    /**
     * Obtiene estadÃ­sticas de uso de categorÃ­as
     */
     ...
     * @param array $filters Filtros de bÃºsqueda
     * @return int NÃºmero total de department_categories
     */
    ...
    /**
     * Validar datos especÃ­ficos de department_categories
    ...
    private function validateDepartmentCategoryData(array $data, $id = null): void
    {
        ...
        if (isset($data['parent_id'])) {
            if ($data['parent_id'] !== null) {
                ...
                // Validar jerarquÃ­a si existe el mÃ©todo
        ...
        if (isset($data['name']) && isset($data['department_id'])) {
            ...
            if (!empty($existing)) {
                // Si es actualizaciÃ³n, verificar que no sea el mismo registro
    ...
    /**
     * Invalidar cache especÃ­fico de department_categories
     */
