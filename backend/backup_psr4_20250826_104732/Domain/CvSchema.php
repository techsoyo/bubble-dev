...
namespace Domain\CvSchema.php\Domain;

/**
 * Schema unificado para CV con template y normalizaciÃ³n
 *
 * Proporciona:
 * - TEMPLATE: estructura base para el formulario
 * - normalize(): mÃ©todo para limpiar y validar datos de entrada
...
class CvSchema
{
  ...
  public const TEMPLATE = [
    ...
    // bt_candidate_routing - InformaciÃ³n de enrutamiento
    'routing' => [
  ...
  /**
   * Plantilla mÃ­nima para prompts de IA
   */
  public const PROMPT_MINIMAL = [
    'nombre' => 'Juan PÃ©rez GarcÃ­a',
    'email' => 'juan.perez@email.com',
    'telefono' => '+34 123 456 789',
    'ubicacion_actual' => 'Madrid, EspaÃ±a',
    'fecha_nacimiento' => '1990-05-15',
    'portfolio' => 'https://juanperez.dev',
    'linkedin' => 'https://linkedin.com/in/juanperez',
    'otras_redes' => ['https://github.com/juanperez'],
    'resumen_profesional' => 'Desarrollador Full Stack con 5 aÃ±os de experiencia en tecnologÃ­as web modernas.',
    ...
    'intereses' => ['TecnologÃ­a', 'Deportes', 'Lectura'],
    'referencias' => [],
    ...
    'puestos_anteriores' => [
      [
        ...
        'responsabilidades' => ['Liderazgo tÃ©cnico', 'Mentoring de desarrolladores junior']
      ]
    ],
    'educacion' => [
      [
        'titulo' => 'IngenierÃ­a InformÃ¡tica',
        'institucion' => 'Universidad Complutense Madrid',
        'fecha_inicio' => '2014-09',
        'fecha_fin' => '2018-06',
        'descripcion' => 'EspecializaciÃ³n en desarrollo web y bases de datos'
      ]
    ],
    'certificaciones' => [
      [
        ...
        'fecha' => '2022-08',
        'descripcion' => 'CertificaciÃ³n en arquitectura cloud'
      ]
    ],
    'proyectos' => [
      [
        'nombre' => 'Sistema de gestiÃ³n de talento',
        'descripcion' => 'Plataforma web para gestiÃ³n de recursos humanos',
  ...
  public static function normalize(array $input): array
  {
    ...
    // Sanitizar campos bÃ¡sicos de texto
    $textFields = [
      'nombre',
      'email',
      'telefono',
    ...
    foreach ($textFields as $field) {
      if (isset($input[$field])) {
        // Solo sanitizar si es string, si es array dejarlo como estÃ¡
  ...
  /**
   * Valida estructura y contenido del CV segÃºn el contrato
   *
   * @param array $data Datos del CV a validar
   * @return array Array de errores, vacÃ­o si todo estÃ¡ correcto ['campo'=>'motivo']
   */
  public static function validate(array $data): array
  {
    ...
    foreach ($dateFields as $field) {
      if (!empty($data[$field]) && !self::isValidDate($data[$field])) {
        $errors[$field] = 'Formato de fecha invÃ¡lido (use YYYY-MM-DD)';
      }
    }
    ...
    foreach ($stringLimits as $field => $limit) {
      if (!empty($data[$field]) && strlen($data[$field]) > $limit) {
        $errors[$field] = "El campo excede el lÃ­mite de {$limit} caracteres";
      }
    }

    // Validar arrays no excedan lÃ­mites
    $arrayLimits = [
      'otras_redes' => 10,
      'soft_skills' => 50,
      'hard_skills' => 50,
    ...
    foreach ($arrayLimits as $field => $limit) {
      if (!empty($data[$field]) && is_array($data[$field]) && count($data[$field]) > $limit) {
        $errors[$field] = "El campo excede el lÃ­mite de {$limit} elementos";
      }
    }
    ...
    if (!empty($data['puestos_anteriores']) && is_array($data['puestos_anteriores'])) {
      foreach ($data['puestos_anteriores'] as $index => $puesto) {
        if (!empty($puesto['fecha_inicio']) && !self::isValidDateYearMonth($puesto['fecha_inicio'])) {
          $errors["puestos_anteriores.{$index}.fecha_inicio"] = 'Formato de fecha invÃ¡lido (use YYYY-MM)';
        }
        if (!empty($puesto['fecha_fin']) && !self::isValidDateYearMonth($puesto['fecha_fin'])) {
          $errors["puestos_anteriores.{$index}.fecha_fin"] = 'Formato de fecha invÃ¡lido (use YYYY-MM)';
        }
      }
    }

    // Validar fechas en educaciÃ³n
    if (!empty($data['educacion']) && is_array($data['educacion'])) {
      foreach ($data['educacion'] as $index => $edu) {
        if (!empty($edu['fecha_inicio']) && !self::isValidDateYearMonth($edu['fecha_inicio'])) {
          $errors["educacion.{$index}.fecha_inicio"] = 'Formato de fecha invÃ¡lido (use YYYY-MM)';
        }
        if (!empty($edu['fecha_fin']) && !self::isValidDateYearMonth($edu['fecha_fin'])) {
          $errors["educacion.{$index}.fecha_fin"] = 'Formato de fecha invÃ¡lido (use YYYY-MM)';
        }
      }
    }
  ...
  /**
   * ValidaciÃ³n de datos mÃ­nimos requeridos para un CV
   */
  public static function validateMinimumData(array $data): array
  {
    $errors = [];

    // Validar campos mÃ­nimos obligatorios
    if (empty($data['nombre'])) {
      $errors['nombre'] = 'El nombre es obligatorio';
    }
    ...
    if (empty($data['telefono']) && empty($data['email'])) {
      $errors['contacto'] = 'Debe proporcionar al menos email o telÃ©fono';
    }
