...
/**
 * Reporting para dashboard con 4 mÃ©tricas.
 * Rutas:
...
function dates(): array
{
    ...
    // Normaliza a lÃ­mites del dÃ­a si vienen
...
/**
 * KPI 1: Total candidatos por departamento (usa el routing mÃ¡s reciente por candidato)
 * Devuelve: { labels: [depName...], series: [count...] }
 */
function kpi_candidates_by_department(PDO $db, ?string $from, ?string $to): array
{
    // Subconsulta para Ãºltimo routing por candidato
...
try {
    ...
    if (!is_array($result) || !isset($result['labels'], $result['series'])) {
        jsend(false, 'Formato de salida invÃ¡lido', $result, 500);
    }
