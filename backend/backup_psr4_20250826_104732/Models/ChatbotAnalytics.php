...
namespace Models\ChatbotAnalytics.php\Models;
...
/**
 * Modelo para el registro de analÃ­ticas del chatbot.
 * Guarda eventos, interacciones y datos de uso con anÃ¡lisis avanzados.
 * 
 * @package Models
...
class ChatbotAnalytics extends BaseModel
{
     ...
     * 
     * Cambios realizados:
     * âž• Campos aÃ±adidos: ['user_id', 'event_type', 'event_data', 'processed']
     * âŒ Campos removidos: ['candidate_id', 'node_id', 'option_id', 'action_type', 'data']
     * ðŸ“Š Total campos fillable: 8
     * 
    ...
    /**
     * Cache para reportes analÃ­ticos pesados (tiempo en minutos)
     */
    private const ANALYTICS_CACHE_TTL = 60;
    private const REPORTS_CACHE_TTL = 180;

    /**
     * Tipos de acciÃ³n vÃ¡lidos para anÃ¡lisis
     */
    ...
    /**
     * Crear nuevo registro de analytics con validaciÃ³n mejorada
     * 
    ...
    public function store($data)
    {
        ...
        if (!in_array($data['action_type'], self::VALID_ACTION_TYPES)) {
            throw new \InvalidArgumentException("Tipo de acciÃ³n invÃ¡lido: {$data['action_type']}");
        }
    ...
    /**
     * Obtener flujo completo de conversaciÃ³n por sesiÃ³n
     * 
    ...
    {
        if (empty($sessionId)) {
            throw new \InvalidArgumentException('Session ID no puede estar vacÃ­o');
        }
        ...
        try {
            ...
            Logger::debug('Flujo de conversaciÃ³n obtenido', [
            ...
            Logger::error('Error al obtener flujo de conversaciÃ³n', [
                'session_id' => $sessionId,
                'error' => $e->getMessage()
            ]);
            throw new \RuntimeException('Error al obtener flujo de conversaciÃ³n: ' . $e->getMessage());
        }
    }

    /**
     * Obtener estadÃ­sticas por nodo
     * 
     * @param array $filters Filtros opcionales (date_from, date_to, node_ids)
     * @return array EstadÃ­sticas agrupadas por nodo
     * @throws RuntimeException Si hay error en BD
     */
    public function getNodeStatistics(array $filters = []): array
    {
        ...
        try {
            ...
            Logger::debug('EstadÃ­sticas de nodos obtenidas', [
                'filters' => $filters,
            ...
            Logger::error('Error al obtener estadÃ­sticas de nodos', [
                'filters' => $filters,
                'error' => $e->getMessage()
            ]);
            throw new \RuntimeException('Error al obtener estadÃ­sticas de nodos: ' . $e->getMessage());
        }
    }

    /**
     * Obtener estadÃ­sticas por opciÃ³n seleccionada
     * 
     * @param array $filters Filtros opcionales
     * @return array EstadÃ­sticas agrupadas por opciÃ³n
     * @throws RuntimeException Si hay error en BD
     */
    public function getOptionStatistics(array $filters = []): array
    {
        ...
        try {
            ...
            Logger::debug('EstadÃ­sticas de opciones obtenidas', [
                'filters' => $filters,
            ...
            Logger::error('Error al obtener estadÃ­sticas de opciones', [
                'filters' => $filters,
                'error' => $e->getMessage()
            ]);
            throw new \RuntimeException('Error al obtener estadÃ­sticas de opciones: ' . $e->getMessage());
        }
    }

    /**
     * Obtener anÃ¡lisis de sesiones con mÃ©tricas avanzadas
     * 
     * @param array $filters Filtros opcionales
     * @return array AnÃ¡lisis de sesiones
     * @throws RuntimeException Si hay error en BD
     */
    public function getSessionAnalytics(array $filters = []): array
    {
        ...
        try {
            ...
            Logger::debug('AnÃ¡lisis de sesiones obtenido', [
                'filters' => $filters,
            ...
            Logger::error('Error al obtener anÃ¡lisis de sesiones', [
                'filters' => $filters,
                'error' => $e->getMessage()
            ]);
            throw new \RuntimeException('Error al obtener anÃ¡lisis de sesiones: ' . $e->getMessage());
        }
    }

    /**
     * Obtener las rutas mÃ¡s frecuentes en el chatbot
     * 
     * @param array $filters Filtros opcionales
     * @param int $limit LÃ­mite de rutas a retornar
     * @return array Rutas mÃ¡s populares
     * @throws RuntimeException Si hay error en BD
     */
    public function getMostPopularPaths(array $filters = [], int $limit = 10): array
    {
        ...
        try {
            ...
            // Obtener secuencias de nodos por sesiÃ³n
            $query = "
     ...
     * 
     * @param array $filters Filtros opcionales
     * @return array MÃ©tricas de abandono
     * @throws RuntimeException Si hay error en BD
     */
    ...
    /**
     * Generar reporte completo de analÃ­ticas
     * 
     * @param array $filters Filtros opcionales
     * @return array Reporte completo con todas las mÃ©tricas
     * @throws RuntimeException Si hay error en BD
     */
    ...
    /**
     * Limpiar datos antiguos de analytics con polÃ­ticas de retenciÃ³n
     * 
     * @param int $daysToKeep DÃ­as a conservar (por defecto 90)
    ...
    {
        if ($daysToKeep < 1) {
            throw new \InvalidArgumentException('Los dÃ­as a conservar deben ser mayor a 0');
        }
    ...
    /**
     * Calcular tasa de conversiÃ³n para un nodo especÃ­fico
     * 
     * @param string $nodeId ID del nodo
     * @param array $filters Filtros aplicados
     * @return float Tasa de conversiÃ³n
     */
    private function calculateNodeConversionRate(string $nodeId, array $filters = []): float
    {
        try {
            ...
            Logger::warning('Error calculando tasa de conversiÃ³n de nodo', [
                'node_id' => $nodeId,
                'error' => $e->getMessage()
            ]);
            return 0.0;
    ...
    /**
     * Calcular tasa de abandono para un nodo especÃ­fico
     * 
     * @param string $nodeId ID del nodo
     * @param array $filters Filtros aplicados
    ...
    /**
     * Calcular mÃ©tricas de rendimiento general
     * 
     * @param array $filters Filtros aplicados
     * @return array MÃ©tricas de rendimiento
     */
    private function calculatePerformanceMetrics(array $filters = []): array
    {
        try {
            ...
            Logger::warning('Error calculando mÃ©tricas de rendimiento', [
                'filters' => $filters,
                'error' => $e->getMessage()
            ]);
     ...
     * @param array $filters Filtros de bÃºsqueda
     * @return int NÃºmero total de chatbot_analytics
     */
    ...
    /**
     * Validar datos especÃ­ficos de chatbot_analytics
    ...
    /**
     * Invalidar cache especÃ­fico de chatbot_analytics
     */
