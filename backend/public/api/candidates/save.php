...
try {
  ...
  if (empty($data['nombre']) || empty($data['email'])) {
    ResponseHelper::error('Faltan campos requeridos: nombre y email', null, 422);
    exit;
  }
  ...
  $stmt = $pdo->prepare("
        INSERT INTO bt_candidates ( name, email, phone, location, date_of_birth,
            ...
            created_at
        ) VALUES ( :name, :email, :phone, :location, :date_of_birth,
            :portfolio_url, :linkedin_url, :professional_summary,
            :soft_skills, :hard_skills, :languages, :interests, :references,
            :availability, :data_source, :cv_original_file, :cv_text_file, :cv_json_file,
            NOW()
  ...
  ResponseHelper::error('Error interno del servidor', $e, 500);
  exit;
}
