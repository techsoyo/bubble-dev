...
$healthData = null;
$healthCheck = @file_get_contents('http://localhost:8000/api/health');
...
$endpoints = [
  ...
  // 4. CompanyController (5 endpoints)
...
$payloads = ['POST' => json_encode(['test' => 'data', 'name' => 'Test Entry', 'email' => 'test@example.com', 'status' => 'active']), 'PUT' => json_encode(['id' => 1, 'name' => 'Updated Test Entry', 'status' => 'updated']), 'PATCH' => json_encode(['status' => 'modified', 'updated_at' => date('Y-m-d H:i:s')])];
...
// Mejorar la función makeRequest con mejor manejo de errores
function makeRequest($url, $method, $payload = null)
{
  ...
  if (in_array($method, ['POST', 'PUT', 'PATCH']) && $payload) {
    ...
    curl_setopt($ch, CURLOPT_HTTPHEADER, ['Content-Type: application/json', 'Accept: application/json', 'Content-Length: ' . strlen($payload)]);
  } else {
...
$errorTests = 0;
$results = [];
// Ejecutar todos los tests (silencioso para interfaz web) 
foreach ($endpoints as $index => $endpoint) {
  ...
  $result = makeRequest($fullUrl, $method, $payload);
  ...
  $results[] = ['index' => $index + 1, 'method' => $method, 'url' => $url, 'fullUrl' => $fullUrl, 'status' => $result['status'] ?? 0, 'statusText' => $statusText, 'category' => $category, 'error' => $result['error'] ?? null, 'response' => $result['response'] ?? null, 'responsePreview' => $responsePreview, 'payload' => $payload];
}
$endTime = microtime(true);
$executionTime = round($endTime - $startTime, 2);
// Limpiar el buffer y empezar la salida HTML ob_end_clean();
?>
<!DOCTYPE html>
<html lang="es">
...
<body>
  <div class="container"> <!-- Header -->
    ...
    <div class="server-status <?php echo $serverStatus; ?>"> <i class="fas fa-<?php echo $serverStatus === 'ok' ? 'check-circle' : 'exclamation-triangle'; ?>"></i> <?php if ($serverStatus === 'ok'): ?> <strong>Servidor funcionando correctamente</strong> - Conectado a localhost:8000 <?php else: ?> <strong>Advertencia:</strong> No se pudo verificar el servidor. Asegúrate de que esté ejecutando: <code>php -S localhost:8000 router.php</code> <?php endif; ?> </div> <!-- Statistics -->
    <div class="stats">
    ...
    <div class="filters">
      <div class="filter-buttons"> <button class="filter-btn all active" onclick="filterResults('all')"> <i class="fas fa-list"></i> Todos (<?php echo $totalTests; ?>) </button> <button class="filter-btn success" onclick="filterResults('success')"> <i class="fas fa-check"></i> Exitosos (<?php echo $successfulTests; ?>) </button> <button class="filter-btn warning" onclick="filterResults('warning')"> <i class="fas fa-exclamation-triangle"></i> Advertencias (<?php echo $warningTests; ?>) </button> <button class="filter-btn error" onclick="filterResults('error')"> <i class="fas fa-times"></i> Errores (<?php echo $errorTests; ?>) </button> </div>
    </div> <!-- Results -->
    <div class="results"> <?php foreach ($results as $result): ?> <div class="endpoint-card <?php echo $result['category']; ?>" data-category="<?php echo $result['category']; ?>">
          <div class="endpoint-header" onclick="toggleDetails(<?php echo $result['index']; ?>)"> <span class="method-badge method-<?php echo strtolower($result['method']); ?>"> <?php echo $result['method']; ?> </span> <span class="endpoint-url"><?php echo $result['url']; ?></span> <span class="status-badge status-<?php echo $result['category']; ?>"> <?php echo $result['statusText']; ?> </span> <i class="fas fa-chevron-down" style="color: #9ca3af; transition: transform 0.3s;"></i> </div>
          <div class="endpoint-details" id="details-<?php echo $result['index']; ?>">
            <div class="detail-section">
              <div class="detail-title"> <i class="fas fa-link"></i> URL Completa </div>
              <div class="detail-content"><?php echo htmlspecialchars($result['fullUrl']); ?></div>
            </div> <?php if ($result['payload']): ?> <div class="detail-section">
                <div class="detail-title"> <i class="fas fa-upload"></i> Payload Enviado </div>
                <div class="detail-content"><?php echo htmlspecialchars($result['payload']); ?></div>
              </div> <?php endif; ?> <?php if ($result['responsePreview']): ?> <div class="detail-section">
                <div class="detail-title"> <i class="fas fa-download"></i> Respuesta del Servidor </div>
                <div class="detail-content"><?php echo htmlspecialchars($result['responsePreview']); ?></div>
              </div> <?php endif; ?>
          </div>
        </div> <?php endforeach; ?> </div> <!-- Footer -->
    <div class="footer"> <i class="fas fa-clock"></i> Ejecutado en <?php echo $executionTime; ?>s | <i class="fas fa-calendar"></i> <?php echo date('Y-m-d H:i:s'); ?> | <i class="fas fa-code"></i> Bubble of Talents API Testing Suite </div>
  </div>
  <script>
