 ...
 *  - Validaciones servidor (formato email, longitudes, fechas, tamaÃ±os listas)
 *  - TransacciÃ³n PDO:
 *      * upsert bt_candidates por email
 *      * limpia e inserta tablas hijas: experiencias, educaciÃ³n, certificaciones (simple), proyectos
 *  - Commit y respuesta
 * Errores controlados:
 *  - 400 INVALID_JSON
...
if (!$isCli) {
    ...
    if ($method !== 'POST') {
        jsonResponse(405, ['success' => false, 'error' => ['code' => 'METHOD_NOT_ALLOWED', 'message' => 'MÃ©todo no permitido', 'details' => (object)[]]]);
    }
...
function validateServer(array $data): array
{
    ...
    if (!empty($data['email']) && !filter_var($data['email'], FILTER_VALIDATE_EMAIL)) {
        ...
        $details['email'] = 'Formato no vÃ¡lido';
    }
    ...
    foreach (($data['puestos_anteriores'] ?? []) as $i => $p) {
        foreach (['fecha_inicio', 'fecha_fin'] as $df) {
            if (!empty($p[$df]) && !$checkDate($p[$df])) {
                $errors[] = "Experiencia[$i].$df formato invÃ¡lido";
    ...
    foreach (($data['educacion'] ?? []) as $i => $e) {
        foreach (['fecha_inicio', 'fecha_fin'] as $df) {
            if (!empty($e[$df]) && !$checkDate($e[$df])) {
                $errors[] = "Educacion[$i].$df formato invÃ¡lido";
    ...
    foreach (($data['proyectos'] ?? []) as $i => $p) {
        foreach (['fecha_inicio', 'fecha_fin'] as $df) {
            if (!empty($p[$df]) && !$checkDate($p[$df])) {
                $errors[] = "Proyecto[$i].$df formato invÃ¡lido";
...
if (!is_array($dataIn)) {
    respondJson(400, false, errorPayload('INVALID_JSON', 'JSON invÃ¡lido'));
}
...
if (!empty($minErrors)) {
    respondJson(422, false, errorPayload('VALIDATION_FAILED', 'Datos mÃ­nimos incompletos', $minErrors));
}
...
if ($vErrors) {
    ...
    respondJson(422, false, errorPayload('VALIDATION_FAILED', 'Violaciones de validaciÃ³n', $aux));
}
